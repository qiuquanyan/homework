...
public class Aggregate extends AbstractDbIterator {
	...
	private Aggregator agtor;
	private DbIterator it;//initial
    /**
     * Constructor.  
     *
    ...
    public void open()
    	...
    	if(this.Gfield!=Aggregator.NO_GROUPING) {
    		ty=this.Child.getTupleDesc().getType(Gfield);
    	}
    	if(this.Child.getTupleDesc().getType(Afield)==Type.INT_TYPE) {
    		this.agtor=new IntAggregator(this.Gfield,ty,this.Afield,this.Aop);
    		//System.out.print("2");
    		...
    		this.agtor=new StringAggregator(this.Gfield,ty,this.Afield, this.Aop);
    		//System.out.print("3");

    	}
    	while(this.Child.hasNext()) {
    		this.agtor.merge(this.Child.next());
    	}
    	//this.Child.close();
    	it = this.agtor.iterator();
    	it.open();
    }
    ...
    public TupleDesc getTupleDesc() {
        // some code goes here
    	TupleDesc tuple = this.Child.getTupleDesc();
        if(this.Gfield==Aggregator.NO_GROUPING) {
        	return new TupleDesc(new Type[] {Type.INT_TYPE},
        			new String[] {tuple.getFieldName(Afield)});     			
        }
        else {
        	
        	return new TupleDesc(new Type[] {tuple.getType(Gfield),Type.INT_TYPE},
        			new String[] {tuple.getFieldName(Gfield),tuple.getFieldName(Afield)});
        }
    }
