...
public class HeapPage implements Page {
    ...
    */
    private int getNumTuples() {        
        // some code goes here
    	int num_=(BufferPool.PAGE_SIZE*8) / (td.getSize() * 8 + 1);
    	//int nums=(BufferPool.PAGE_SIZE/td.getSize())/8+1;
    	return num_;
    }
    ...
    public void deleteTuple(Tuple t) throws DbException {
    	...
    	assert t !=null;
    	RecordId del_rec=t.getRecordId();
    	if(del_rec!=null && this.pid.equals(del_rec.getPageId())) {
    		for(int i=0;i<this.numSlots;i++) {
    			if(this.getSlot(i)&& t.getRecordId().equals(this.tuples[i].getRecordId())) {
    				this.setSlot(i, false);
    				this.tuples[i]=null;
    				return;
    			}
    		}
    		throw new DbException("error1");
    	}
    	throw new DbException("error2");
    }
    ...
    public void addTuple(Tuple t) throws DbException {
    	...
    	throw new DbException("no empty");
    }
     ...
     */
    //此处与操作结果应当判断非0则返回true，因为它不一定是1，还有可能是其他值
    public boolean getSlot(int i) {
    	...
    	int byte_num=i/8;
    	int bit_num=i%8;
    	if((header[byte_num]&(0x1<<bit_num))!=0)
    		return true;
    	else
    		return false;
    }
    ...
    private void setSlot(int i, boolean value) {
    	...
    	if(i<this.numSlots) {
    		int head_num=i/8;
    		int off_num=i%8;
    		byte mask=(byte)(0x1<<off_num);
    		if(value) {
    			this.header[head_num] |=mask;
    		}
    		else {
    			this.header[head_num] &=~mask;
    		}
     ...
     */
    private class Tuple_return implements Iterator<Tuple>{
    	
    	private int tuple_index= 0;
    	private int bit_num=0;
    	private int used_num=getNumTuples()-getNumEmptySlots();
		...
		public boolean hasNext() {
			...
			if (tuple_index<getNumTuples() && bit_num<used_num) {
    			return true;
    		}
    		return false;
		}
    	...
    	public Tuple next() {
    		if(hasNext()) {
    			while (true) {
    				if(getSlot(tuple_index))
    					break;
    				tuple_index+=1;
    			}
    			bit_num+=1;
    			return tuples[tuple_index++];
